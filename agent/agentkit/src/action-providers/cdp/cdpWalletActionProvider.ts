...
map where the key is the arg name and the value is the arg value. Encode uint/int/bytes/string/address values as strings, boolean values as true/false. For arrays/tuples, encode based on contained type.`,
  ...
  async deployContract(
    ...
    try {
    const version = args.solidityVersion;
    if (!(version in SolidityVersions)) {
        throw new Error(`Unsupported Solidity version: ${version}`);
    }
    const solidityVersion = SolidityVersions[version as keyof typeof SolidityVersions];
