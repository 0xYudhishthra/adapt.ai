...
import { ERC20_TOKENS_BY_NETWORK } from "../erc20/constants";
import { erc20ActionProvider } from "../erc20/erc20ActionProvider";
  ...
  async getWalletDetails(
    ...
    try {
      ...
      const erc20Balances = await Promise.all(
        (ERC20_TOKENS_BY_NETWORK[network.networkId || "base-sepolia"] || []).map(async token => {
          try {
            const balanceResult = await erc20ActionProvider().getBalance(walletProvider, {
              contractAddress: token.address,
            });

            // Create new Decimal instances for both balance and divisor
            return `${new Decimal(balanceResult.toString()).div(new Decimal(10).pow(token.decimals)).toFixed(2)} ${token.symbol}`;
          } catch (error) {
            console.error(`Error fetching ${token.symbol} balance:`, error);
            return `0.000000 ${token.symbol}`;
          }
        }),
      );
